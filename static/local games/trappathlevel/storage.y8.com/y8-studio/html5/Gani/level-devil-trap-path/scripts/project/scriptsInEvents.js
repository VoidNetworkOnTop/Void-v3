...
window.idAsyncInit = function() {
	...
	ID.init({
        ...
        appId : "66d6a96ccd32e6efea2fd34c"
    });
};
    ...
    adBreak({
        ...
        adBreakDone: (placementInfo) => {
            ...
            if(placementInfo.breakStatus == "frequencyCapped"){updateTextRewardPanel()};
			 if(placementInfo.breakStatus == "other"){updateTextRewardPanel()};
        },
    });
}
...
const scriptsInEvents = {
	...
	async Y8api_Event3_Act1(runtime, localVars)
	{
		...
		ID.GameAPI.Leaderboards.save({'table':'Leaderboard','points':localVars.score_}, function(data){
		                console.log(data);
						runtime.globalVars["isScoreSubmitted"] = 1;
		             });
	},
	...
	async Y8api_Event5_Act1(runtime, localVars)
	{
		...
		console.log("localVars.name_ "+ localVars.name_);
		console.log("localVars.key_ "+ localVars.key_);
	},
	...
	async Y8api_Event16_Act1(runtime, localVars)
	{
		ID.openProfile();
	},

	async Y8api_Event17_Act1(runtime, localVars)
	{
		...
		function sentImageToProfile(_image)
		{
			ID.submit_image(_image, function(response){
				...
				runtime.globalVars["isScreenSubmitted"] = 1;
		      });
		}
		
	},

	async Y8api_Event18_Act2(runtime, localVars)
	{
	...
	async Y8api_Event19_Act1(runtime, localVars)
	{
		if (window.ID && ID.isVisible()) {
		     	runtime.globalVars["isY8menu"] = 1;
		    }
			else
			{
				runtime.globalVars["isY8menu"] = 0;
			}
	},

	async Y8api_Event20_Act1(runtime, localVars)
	{
		
	},

	async Y8api_Event21_Act2(runtime, localVars)
	{
		
	},

	async Y8api_Event22_Act2(runtime, localVars)
	{
		
	},

	async Y8api_Event39_Act1(runtime, localVars)
	{
		console.log("localVars.score_ "+localVars.score_);
		var scrtemp = 0;
		scrtemp = localVars.score_;
		ID.login(function(response){
			console.log("response "+response)
				if(response.status === 'ok')
				{
			...
			function submitScr()
			{
		ID.GameAPI.Leaderboards.save({'table':'Leaderboard','points':scrtemp}, function(data){
		                console.log(data);
						runtime.globalVars["isScoreSubmitted"] = 1;
		             });
			}
